...
\begin{align*}
    ...
    \rho \in \Env & = \alert<2>{\Value}^{\Var} \only<1>{\alert{= \Set{\rho : \Var \to \Value}}} \\
...
\begin{frame}{Abstracting Abstract Machines (AAM)}
  \begin{itemize}
  \item The \alt<2->{abstract}{concrete} transition relation \alt<2->{$\abst\varsigma \abst\step {\abst\varsigma}'$}{$\varsigma \step \varsigma'$}
  \item Current state: \(\alt<2->{\abst\varsigma}{\varsigma} = ({so}, {\alt<2->{\abst\rho}{\rho}}, {\alt<2->{\abst\sigma}{\sigma}}, {\alt<2->{\abst\kappa}{\kappa}}) \)
\item Next state: \( {\alt<2->{\abst\varsigma}{\varsigma}}' = ({so}', {\alt<2->{\abst\rho}{\rho}}', {\alt<2->{\abst\sigma}{\sigma}}', {\alt<2->{\abst\kappa}{\kappa}}') \)
  \end{itemize}
  \footnotesize
  \[
  \begin{array}{llllll}
    so & \text{side condition} & {so}' & {\alt<2->{\abst\rho}{\rho}}' & {\alt<2->{\abst\sigma}{\sigma}}' & {\alt<2->{\abst\kappa}{\kappa}}' \\ \hline
    e_1 \update e_2 & \alt<2->{\abst{a} = \abst\eta_{\abst\varsigma}(e_1)}{a = \eta_\varsigma(e_1)} & - & \alt<2->{\abst\rho}{\rho} & \alt<2->{\alert<2>{\alt<2->{\abst\sigma}{\sigma}[a \mapsto \alt<2->{\abst\sigma}{\sigma}(a) \sqcup \abst\eta_{\abst\varsigma}(e_2)]}}{\sigma[a \mapsto \eta_\varsigma(e_2)]} & {\alt<2->{\abst\kappa}{\kappa}} \\
    \kw{while}\; e\; s & \alt<2->{\alert<3>{\true \in \gamma(\abst\eta_{\abst\varsigma}(e))}}{\eta_\varsigma(e) = \true} & - & \alt<2->{\abst\rho}{\rho} & \alt<2->{\abst\sigma}{\sigma} & \sTag{stmtK}{(s, \sTag{whileK}{(e, s,\alt<2->{\abst\kappa}{\kappa})})} \\
    \kw{while}\; e\; s & \alt<2->{\alert<3>{\false \in \gamma(\abst\eta_{\abst\varsigma}(e))}}{\eta_\varsigma(e) = \true} & - & \alt<2->{\abst\rho}{\rho} & \alt<2->{\abst\sigma}{\sigma} & {\alt<2->{\abst\kappa}{\kappa}} \\
    - & \alt<2->{\abst\kappa}{\kappa} = \sTag{stmtK}{(s,\alt<2->{\abst\kappa}{\kappa}_r)} & s & \alt<2->{\abst\rho}{\rho} & \alt<2->{\abst\sigma}{\sigma} & {\alt<2->{\abst\kappa}{\kappa}_r}
  \end{array}
\]
  ...
  \begin{columns}[t]
    ...
    \begin{column}{0.55\textwidth}
      \only<beamer>{
        ...
        \item<6-> The model checker finds a bad path, reify it using \fn{Pre}
