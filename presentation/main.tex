...
\end{column}
\end{columns}
Note: Widening operators are more general than Galois Connections.\footcite{cousot92comparing}
...
\begin{frame}{Other methods partitioning the state}
  ...
  \item Miné\footcite{mine2011static} extends an abstract interpreter for sequential programs to parallel programs with statically known threads and locks
    \begin{itemize}
    \item He partitions the state space based on the status of threads (waiting on a lock $l$, yielding, ready) and status of locks (held by thread $t$, free)
