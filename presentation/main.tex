...
\begin{frame}{SIMPL\footnote{simple introductory made-up programming language} Syntax}
  ...
  \begin{align*}
    fd \in \ns{FunDef} \eDEF & \kw{def}\, f(\vo{x}) = s;\,\kw{return}\, e \\
    e \in \Exp \eDEF & x \eOR n \eOR b \eOR e_1 \oplus e_2 \eOR *x \\
    s \in \Stmt \eDEF & s;\,s \eOR \kw{abort} \\
    \eOR & \kw{if}\, e\; s_1 \; \kw{else} \; s_2 \\
    \eOR & \kw{while}\; e\; s \\
    \eOR & \kw{let}\; x := e \;\kw{in}\;s \\
    \eOR & \kw{letref}\; x := \kw{new}\;e \;\kw{in}\;s \\
    \eOR & e_1 \update e_2 \eOR x := e \\
    \eOR & x := f(\vo{e})
  \end{align*}
\end{frame}
...
\begin{frame}{Collecting semantics}
  Abstract machine semantics gives us a state transition system $(\State, I, R)$ where \pause $\State$ is the set of states, \pause $I \subset \State$ is the set of initial states, \pause $R \subset \State \times \State$ is the transition relation. \pause Let
  \[ F : \pw(\State) \monarrow \pw(\State),\, F(X) = \Set{y \vert (x, y) \in R \wedge x \in X}. \]\pause
...
\begin{frame}{Abstract Garbage Collection\footcite{might2006improving}}
  \todo{Explain the analogy and how it benefits}
\end{frame}
...
\begin{frame}{Binary product abstract domain functor}
  ...
  \todo{\footcite{cousot2005astree,cousot2007varieties,cousot2006combination}}
\end{frame}
...
\begin{frame}{Extending non-parallel analysis to multithreaded programs}
  \todo{\footcite{mine2011static}}
\end{frame}
