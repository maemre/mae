...
\end{itemize}
\todo{Explain what invariants are, what's special about low-level programs, and how these techniques are good for them (both as need and amenability)}
\end{itemize}
...
\begin{frame}{SIMPL\footnote{simple introductory made-up programming language} Syntax}
  ...
  \begin{align*}
    ...
    e \in \Exp \eDEF & x \eOR n \eOR b \eOR e_1 \oplus e_2 \eOR \alert<3>{*x} \\ % \eOR \alert<1>{\kw{choose}\;e_1\,e_2} \\
...
\begin{frame}{Collecting semantics}
  ...
  \[ F : \pw(\State) \alert<-6>{\monarrow} \pw(\State),\, F(X) = \Set{y \vert x \step y \wedge x \in X}. \]\pause
...
\begin{frame}{Abstract interpretation}
  \todo{move this to the beginning}
  \begin{itemize}[<+->]
...
\begin{frame}{Relating lattices}
  ...
  \begin{columns}[t]
    ...
    \item<6-> $\alpha$ and $\gamma$ are \emph{sound}: we don't leave anything behind when we go abstract then come back to the concrete \todo{Clean up}
...
\begin{frame}[fragile]{Safe simulations}
  \small
  \todo{reify $\varsigma$s to actual states, then abstract them}
  \vspace{-1em}
  \begin{itemize}[<+->]
...
\begin{frame}{Previous work on improving/designing abstract interpreters}
  ...
  \item Abstracting abstract machines\footcite{van2010abstracting,van2012systematic}
...
\begin{frame}{Relationship with model checking}
  \small\todo{try finding a better DFA equation}
  \begin{itemize}[<+->]
...
\begin{frame}{Predicate abstraction}
  \todo{why is this useful for low-level software?}
  \begin{itemize}[<+->]
...
\begin{frame}{Counterexample-Generated Abstraction Refinement (CEGAR)}
  ...
  \item \todo{Announce: ``We will go through an example''}
  \end{itemize}
\end{frame}

\begin{frame}{CEGAR Example (from SLAM project)}
  \footnotesize\todo{Drop this slide or make it simple}
  \begin{enumerate}[<+->]
...
\begin{frame}{What is challenging about control software?}
  \todo{what is challenging here?}
  \todo{what properties are interesting invariants?}
  \todo{what about (or restrictions of) these programs make them more amenable for analysis?}
...
\begin{frame}{Dynamic state partitioning for automata\footcite{jeannet_dynamic_1999}}
  \todo{try showing an automaton and a state split from the paper}
  \begin{itemize}[<+->]
...
\begin{frame}{Other methods for partitioning the state}
  \todo{fewer words, just general concepts}
  \begin{itemize}[<+->]
...
\begin{frame}{A plethora of domains}
  \todo{explain why we need this many}
  \footnotesize
  \begin{tightcenter}
  \begin{tabular}{lcc}
  ...
  \item How do we choose which domains to use?\todo{skip this}
...
\begin{frame}[fragile]{Reduced products\footcite{cousot1979systematic}}
  ...
  \begin{figure}
    ...
    \begin{subfigure}[b]{0.45\textwidth}\tiny
      ...
      \[\begin{tikzcd}
        &  & \top &  \\
        \only<2>{\alert{Null,Cyc} \arrow[rru, no head]} & \mathit{Null,Acyc} \arrow[ru, no head] & \mathit{NotNull,Cyc} \arrow[u, no head] & \mathit{NotNull,Acyc} \arrow[lu, no head] \\
        &  & \bot \arrow[ru, no head] \arrow[lu, no head] \arrow[u, no head] \only<2>{\arrow[llu, no head]} & 
      \end{tikzcd}\]
    \caption{\small \alt<3->{The reduced product of $Null$ and $Cyc$}{The Cartesian product $Null \times Cyc$}}
  \end{subfigure}
\end{figure}
\begin{itemize}
\item<4-> Reduced products have the same concretization. They eliminate impossible (spurious) configurations
\item<5-> There is no general (algorithmic) way to derive them
\item<6-> But iterative reductions can provide an approximation
\end{itemize}
...
\end{itemize}
  ...
  \item Also, Nelson-Oppen combination used in SMT solvers can be used for computing iterative reductions efficiently and separately, even if the decision procedures share their signatures.
  \end{itemize}
\end{frame}
...
\begin{frame}{Widening for Control-Flow\footcite{hardekopf2014widening}}
  \todo{merge from here to monadic a.i. to a single slide}
  \footnotesize
  \begin{columns}
